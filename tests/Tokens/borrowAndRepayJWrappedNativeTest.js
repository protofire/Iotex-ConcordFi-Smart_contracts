...
async function borrowNative(gToken, borrower, borrowAmount, opts = {}) {
  await send(gToken, "harnessFastForward", [1]);
  return send(gToken, "borrowNative", [borrowAmount], { from: borrower });
}
...
async function repayBorrowNative(gToken, borrower, repayAmount) {
  await send(gToken, "harnessFastForward", [1]);
  return send(gToken, "repayBorrowNative", [], {
    from: borrower,
    value: repayAmount,
  });
}
...
async function repayBorrowBehalfNative(gToken, payer, borrower, repayAmount) {
  await send(gToken, "harnessFastForward", [1]);
  return send(gToken, "repayBorrowBehalfNative", [borrower], {
    from: payer,
    value: repayAmount,
  });
}
...
describe("CWrappedNative", function () {
  ...
  describe("borrowNative", () => {
    ...
    it("returns success from borrowFresh and transfers the correct amount", async () => {
      const beforeBalances = await getBalances([gToken], [borrower]);
      await fastForward(gToken);
      const result = await borrowNative(gToken, borrower, borrowAmount);
      const afterBalances = await getBalances([gToken], [borrower]);
      expect(result).toSucceed();
      expect(afterBalances).toEqual(
        await adjustBalances(beforeBalances, [
  ...
  describe("borrow", () => {
    ...
    it("reverts if error returned from borrowFresh", async () => {
      await expect(
        borrow(gToken, borrower, borrowAmount.plus(1))
    ...
    it("returns success from borrowFresh and transfers the correct amount", async () => {
      ...
      expect(afterBalances).toEqual(
        await adjustBalances(beforeBalances, [
          [gToken, "borrows", borrowAmount],
          [gToken, "cash", -borrowAmount],
          [gToken, borrower, "cash", borrowAmount],
          [gToken, borrower, "avax", -(await avaxGasCost(result))],
          [gToken, borrower, "borrows", borrowAmount],
        ])
      );
    });
  ...
  describe("repayBorrowNative", () => {
    ...
    it("reverts if interest accrual fails", async () => {
      await send(gToken.interestRateModel, "setFailBorrowRate", [true]);
      await expect(
        repayBorrowNative(gToken, borrower, repayAmount)
      ).rejects.toRevert("revert INTEREST_RATE_MODEL_ERROR");
    });
    ...
    it("returns success from repayBorrowFresh and repays the right amount", async () => {
      ...
      expect(
        await repayBorrowNative(gToken, borrower, repayAmount)
      ).toSucceed();
    ...
    it("reverts if overpaying", async () => {
      ...
      await expect(
        repayBorrowNative(gToken, borrower, tooMuch)
      ).rejects.toRevert("revert subtraction underflow");
    });
  });
  ...
  describe("repayBorrow", () => {
    beforeEach(async () => {
      ...
      await send(
        gToken.underlying,
        "approve",
        [gToken._address, repayAmount.multipliedBy(2)],
        { from: borrower }
      );
    });
