...
async function fillMasterChef(gToken, amount) {
  const masterChefAddress = await call(gToken, "masterChef", []);
  const masterChef = await saddle.getContractAt(
    "MasterChef",
    masterChefAddress
  );
  await send(gToken.underlying, "transfer", [masterChefAddress, amount]);
  await send(masterChef, "harnessSetUserAmount", [0, gToken._address, amount]);
}

describe("GToken", function () {
  let gToken, root, minter, borrower, benefactor, accounts;
  beforeEach(async () => {
  ...
  describe("borrowFresh", () => {
    ...
    it("proceeds if gTroller tells it to", async () => {
      await fillMasterChef(gToken, borrowAmount);
      expect(await borrowFresh(gToken, borrower, borrowAmount)).toSucceed();
    });
    ...
    it("transfers the underlying cash, tokens, and emits Transfer, Borrow events", async () => {
      ...
      expect(
        await balanceOf(gToken.underlying, masterChefAddress)
