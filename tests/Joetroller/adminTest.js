...
describe("admin / _setPendingAdmin / _acceptAdmin", () => {
  ...
  beforeEach(async () => {
    ...
    gTroller = await deploy("Unitroller");
  });
  ...
  describe("pendingAdmin()", () => {
    it("should return correct pending admin", async () => {
      expect(await call(gTroller, "pendingAdmin")).toBeAddressZero();
    });
  });
  ...
  describe("_setPendingAdmin()", () => {
    it("should only be callable by admin", async () => {
      expect(
        await send(gTroller, "_setPendingAdmin", [accounts[0]], {
          from: accounts[0],
        })
    ...
    it("should properly set pending admin", async () => {
      ...
      // Check admin stays the same
      expect(await call(gTroller, "admin")).toEqual(root);
      expect(await call(gTroller, "pendingAdmin")).toEqual(accounts[0]);
    });
    ...
    it("should properly set pending admin twice", async () => {
      ...
      expect(
        await send(gTroller, "_setPendingAdmin", [accounts[1]])
      ).toSucceed();

      // Check admin stays the same
      expect(await call(gTroller, "admin")).toEqual(root);
      expect(await call(gTroller, "pendingAdmin")).toEqual(accounts[1]);
    });

    it("should emit event", async () => {
      const result = await send(gTroller, "_setPendingAdmin", [accounts[0]]);
  ...
  describe("_acceptAdmin()", () => {
    it("should fail when pending admin is zero", async () => {
      expect(await send(gTroller, "_acceptAdmin")).toHaveTrollFailure(
        "UNAUTHORIZED",
    ...
    it("should succeed and set admin and clear pending admin", async () => {
      ...
      expect(
        await send(gTroller, "_acceptAdmin", [], { from: accounts[0] })
      ).toSucceed();

      // Check admin stays the same
      expect(await call(gTroller, "admin")).toEqual(accounts[0]);
      expect(await call(gTroller, "pendingAdmin")).toBeAddressZero();
    });
    ...
    it("should emit log on success", async () => {
      ...
      const result = await send(gTroller, "_acceptAdmin", [], {
        from: accounts[0],
      });
